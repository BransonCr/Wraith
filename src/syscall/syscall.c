...
static const char *const syscall_names[] = {
    ...
    //all dem syscalls bomba
};

enum { syscall_names_count = sizeof syscall_names / sizeof syscall_names[0] };

const char *syscall_name(uint16_t number) {
    if (number >= syscall_names_count) return NULL;

    assert (number < syscall_names_count);
...
bool syscall_number(const char *name, uint16_t *out) {
    ...
    for (uint32_t index = 0; index < syscall_names_count; index++) {

        if (syscall_names[index] == NULL) continue;

        if (strcmp(syscall_names[index], name) == 0) {
            *out = (uint16_t)index;
            return true;
        }
    }
